...
#Example: perl NetlstFrmt_echo_5_cascading.pl -v c432_clk_opFF_modelsim.v -s c432_clk_opFF.dspf -l glitch_osu018_stdcells_correct_vdd_gnd.sp -c 250 -t 180 -m c432_clk_opFF


#clk frequency in MHz

#Modifications:
#Sim time extended to 6.5 cycles so as to measure output at 2 consequent edges. Measure FF1 and 2 output at one edge (5th cycle), measure FF3 at the following edge (6th cycle). Write out the 6th clk edge measurements to a different file. An output file glitch_report_outputs_cascading_*.csv will be created in which the outputs will be written- Jan 23 2014

#Modified PWL statements for the input nodes to simulate for. 5.5 clk cycles instead of 6.5 cycles. chahged sim_time, fall_from and fall_to times. --Jan 10 2014
...
$fall_from=(5*$clk_period); #defining fall time window
$fall_to= 5.2*$clk_period;

$fall_from2=(6*$clk_period); #defining fall time window
$fall_to2= 6.2*$clk_period;
...
print SIM ".param change_time2='(half_clk_period/2)'\n";

print SIM ".param k_minus_3='(half_clk_period/3)'\n";
...
print SIM ".param k_plus1= '(k_cycle + change_time2 )'\n";
print SIM ".param k_plus1_rise = '(k_plus1 + 100ps)'\n";

print SIM ".param k_plus2= '(k_cycle + clk_period )'\n";
print SIM ".param k_plus2_rise = '(k_plus2 + 100ps)'\n";
...
##     print SIM "\n\nV$i $new 0 PWL( 0 ##$new\_reference_1## k_plus1 ##$new\_reference_1## k_plus1_rise ##$new\_reference_2## $sim_time ##$new\_reference_2##)\n";
  ...
  ##   print SIM "\n\nV$i $new 0 PWL( 0 ##$new\_reference_minus4##  k_minus_3 ##$new\_reference_minus4## k_minus_3rise ##$new\_reference_minus3## k_minus_2 ##$new\_reference_minus3## k_minus_2rise ##$new\_reference_minus2## k_minus_1 ##$new\_reference_minus2## k_minus_1rise ##$new\_reference_minus1## k_cycle ##$new\_reference_minus1## k_cycle_rise ##$new\_reference_1## k_plus1 ##$new\_reference_1## k_plus1_rise ##$new\_reference_2## $sim_time ##$new\_reference_2##)\n";
     ...
     print SIM "\n\nV$i $new 0 PWL( 0 ##$new\_reference_minus3##  k_minus_2 ##$new\_reference_minus3## k_minus_2rise ##$new\_reference_minus2## k_minus_1 ##$new\_reference_minus2## k_minus_1rise ##$new\_reference_minus1## k_cycle ##$new\_reference_minus1## k_cycle_rise ##$new\_reference_1## k_plus1 ##$new\_reference_1## k_plus1_rise ##$new\_reference_2## k_plus2 ##$new\_reference_2## k_plus2_rise ##$new\_reference_3## $sim_time ##$new\_reference_3##)\n";
...
foreach $i(0 .. $#to_ff)
{
 ...
 $measure_at_falling_edge2.="meas tran ff_op_cascading_$i MAX v(X$module.$to_ff[$i]:Q) from=$fall_from2"."s"." to=$fall_to2"."s\n";
}
...
$measure_at_falling_edge2=~s/\[/_/g;
$measure_at_falling_edge2=~s/\]/_/g;
$measure_at_falling_edge2=~s/\//_/g;



#write this to the reference_spice file
print SIM $measure_at_falling_edge;
print SIM "\n\n";
print SIM $measure_at_falling_edge2;
print SIM "\n\n***************** saving the outputs ****************\n";
...
foreach $i(0 .. $#to_ff)
 {
 $new="ff_op_cascading_".$i;

 if ($i == 0) 
  {
   print SIM 'echo "$&'.$new."\" , ";
   print SIM '> glitch_report_outputs_cascading_'."##deck_num##.csv".'  $$New file'."\n"
   } 
  else  
  {
   
   print SIM 'echo "$&'.$new."\" , ";
   print SIM '>> glitch_report_outputs_cascading_'."##deck_num##.csv".'  $$Appending to the file'."\n"
  }
 }
