#Example: perl deckgen_remote_seed_cascading.pl -s reference_spice.sp -l glitch_osu018_stdcells_correct_vdd_gnd.sp -r c499_clk_opFF_reference_out/tool_reference_out.txt -n 1 -m c499_clk_opFF -f /home/users/nanditha/Documents/utility/c499_yuva_new -g 27 -d 2 -c 100 -i 1.42061344093991e-09 -o 1 
...
#||($_=~m/\.ic/)
	   ...
	   ($temp1,$pinname)=split(" ",$_);
           ...
           foreach $index( 0 .. $#temp)
		      ...
		      $new=~s/##$pinname\_reference_3##/$ref3/g;
...
#Appending to the RTL_cascading reference output file, which contains outputs of the extra clk cycles that were picked by this script randomly
open(IM,">>$folder/$module\_reference_out/RTL_cascading.csv");
print IM "\n";
print IM "$deck_num,$cycle,$glitch_location,$random_gate,$rand_gate,";
@temp=split (" ",$next2_cycle);
$start=$#temp-$num_opt+1;
foreach $index( $start .. $#temp)
  {
