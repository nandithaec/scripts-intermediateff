...
backup_dir_cascading = '%s/backup_spice_decks_cascading' %(path)
...
if os.path.exists(backup_dir_cascading):
	shutil.rmtree(backup_dir_cascading)

if not os.path.exists(backup_dir_cascading):
	os.mkdir(backup_dir_cascading)
...
#########################################repeat_deckgen copied starting from here#######################################
	...
	if os.path.isfile("%s/%s_reference_out/RTL_cascading.csv" %(path,module)):
		print "****Removing the existing RTL_cascading.csv file in folder %s_reference_out ****\n" %(module)
		os.remove("%s/%s_reference_out/RTL_cascading.csv" %(path,module))
	...
	#Similarly for the cascading case
	fout = open('%s/%s_reference_out/RTL_cascading.csv' %(path,module), 'w')
	fin = open('%s/%s_reference_out/RTL_backup.csv' %(path,module), 'r')
...
#This means, glitch "can" occur before the input changes in the clk period as well. So, force the glitch to start only after input has changed
		...
		#glitch is being inserted at the 4th clk cycle
		unif=random.uniform(0,0.85*clk_period) 
		rand_glitch= (3.67*clk_period) +  unif #arrival_clk + initial_clk should add up to 3.5+0.15=3.65. 1 period-0.15=0.85
...
#perl deckgen_remote_seed.pl -s reference_spice.sp -l glitch_osu018_stdcells_correct_vdd_gnd.sp -r decoder_behav_pnr_reference_out/tool_reference_out.txt -n 1 -m decoder_behav_pnr -f /home/user1/simulations/decoder -g 27 -d 2 -c 10 -i 1.42061344093991e-09 -o 1 
		...
		os.system('perl %s/deckgen_remote_seed_cascading.pl -s %s/reference_spice.sp -l %s/glitch_%s -r %s/%s_reference_out/tool_reference_out.txt -n %d -m %s -f %s  -o %s -g %s -d %s -c %s -i %s' %(path,path,path,std_lib,path,module,loop_var,module,path,loop,rand_gate,rand_drain,rand_clk,rand_glitch))
...
################################################################################################################
	...
	os.system('python %s/python_GNUparallel_ngspice_remote_yuva_echo_cascading.py -n %s -d %s -o %s -p %s' %(path,num_at_a_time,design_folder,loop,path))
	...
	print "Comparing the RTL and spice outputs for the next clk cycle for the cascaded case\n"
	os.system('python %s/python_compare_remote_seed_cascading.py -m %s -f %s -n %s -t %s -l %d' %(path,module,path,num_at_a_time,tech,loop))
...
shutil.copy('%s/glitch_%s' %(path,std_lib), '%s/backup_spice_decks_cascading' %path )
shutil.copy('%s/tsmc018.m' %path, '%s/backup_spice_decks_cascading' %path )
...
os.system('python  %s/python_count_flips_remote_seed_cascading.py -f %s  -n %s  --group %s -s %s' %(path,path,num,num_at_a_time,seed))  #To save the seed to results file
...
#Add the details of number of DFFs to the cascaded case
fa=open('/%s/subcktinstances.sp' %path, 'r')
fb=open('/%s/spice_results/count_flips_summary_cascading.csv' %path, 'a+')
read=fa.readlines()
filelen=len(read)
fb.writelines(read[filelen-3])
fb.writelines(read[filelen-2])
