...
#This file is a duplicate of the non-cascaded file, except that the next cycle results are counted. Non-cascaded and cascaded, both need to be executed- Jan 23 2014
...
fw = open('%s/spice_results/spice_rtl_difference_summary_cascading.csv' %(path), 'w')
summary_writer = csv.writer(fw)
fd = open('%s/spice_results/spice_rtl_difference_cascading_1.csv' %(path), 'r') #Open in read mode
...
for num in range(1,(num_of_loops+1)): 
	fr = open("%s/spice_results/spice_rtl_difference_cascading_%d.csv" %(path,num), "r")
...
fflip = open('%s/spice_results/count_flips_summary_cascading.csv' %path, 'wb')
fd = open('%s/spice_results/spice_rtl_difference_summary_cascading.csv' %path, 'rb') #Open in read mode
diff_file = csv.reader(fd)
...
if (num_of_multiple_flips > 0):
	...
	probability=0.0
print '\nCascading case: Probability of multiple flips given atleast one flip is:%d/%d = %f\nthat is: %f percent\n' %(num_of_multiple_flips,atleast_one_flip,probability,probability*100)
